...
async function loadAfaPricing() {
  try {
    const effectiveRole = userData?.role === 'admin' ? 'super_agent' : (userData?.role || 'client');

    const { data: prices } = await supabase.from('pricing').select('price').eq('role', effectiveRole).eq('product', 'afa_normal').single();

    if (prices) {
        afaNormalPrice = parseFloat(prices.price);
