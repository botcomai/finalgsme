...
function hardRefresh() {
    ...
    if(document.getElementById('tab-commission-admin').classList.contains('active')) return loadCommissionRequests();
...
async function loadOverviewStats() {
    try {
        ...
        // 1. Total (Real) Users - Excluding admins
        const { count: userCount, error: userErr } = await supabase
            .from('users')
            .select('*', { count: 'exact', head: true })
            .not('role', 'eq', 'admin');
        document.getElementById("metric-real-users").innerText = userCount || 0;
        ...
        // 5. Global SMS Balance
        fetchAdminSmsBalance();

        // 6. Global Order Fulfillment Matrix (Real-time counts)
        try {
            const { count: allOrdersCount } = await supabase.from('orders').select('*', { count: 'exact', head: true });
            const { count: pendingCount_f } = await supabase.from('orders').select('*', { count: 'exact', head: true }).eq('status', 'false');
            const { count: pendingCount_p } = await supabase.from('orders').select('*', { count: 'exact', head: true }).eq('status', 'pending');
            const { count: completedCount } = await supabase.from('orders').select('*', { count: 'exact', head: true }).eq('status', 'true');
            const { count: processingCount } = await supabase.from('orders').select('*', { count: 'exact', head: true }).eq('status', 'Processing');
            const { count: refundedCount } = await supabase.from('orders').select('*', { count: 'exact', head: true }).eq('status', 'Refunded');
            
            const { count: transitCount } = await supabase.from('orders').select('*', { count: 'exact', head: true }).ilike('status', '%transit%');
            const { count: receivedCount } = await supabase.from('orders').select('*', { count: 'exact', head: true }).ilike('status', '%received%');
            const { count: waitingCount } = await supabase.from('orders').select('*', { count: 'exact', head: true }).eq('status', 'Waiting');

            const totalPending = (pendingCount_f || 0) + (pendingCount_p || 0) + (waitingCount || 0);

            document.getElementById("overview-count-all").innerText = allOrdersCount || 0;
            document.getElementById("overview-count-pending").innerText = totalPending;
            document.getElementById("overview-count-completed").innerText = (completedCount || 0) + (receivedCount || 0);
            document.getElementById("overview-count-processing").innerText = (processingCount || 0) + (transitCount || 0);
            document.getElementById("overview-count-refunded").innerText = refundedCount || 0;
        } catch (err) {
            console.warn("Matrix fetch failed:", err);
        }

        // 7. Comprehensive Totals (Revenue, Profit, Liability)
...
async function fetchComprehensiveTotals() {
    try {
        // 1. Total User Balances (Liability)
        const { data: balances } = await supabase.from('users').select('wallet_balance');
        let totalBalances = 0;
        if (balances) {
            balances.forEach(u => totalBalances += parseFloat(u.wallet_balance || 0));
        }
        document.getElementById('metric-user-balances').innerText = "₵" + totalBalances.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });

        // 2. Lifetime Revenue & Profit
        // We need the orders to calculate profit since base_cost is not in the DB schema for each order
        // We will fetch all completed orders.
        const { data: orders } = await supabase.from('orders').select('network, plan, bundle, amount, wholesale_cost').eq('status', 'true');
        
        let totalRevenue = 0;
        let totalProfit = 0;

        if (orders) {
            // Efficiency: Ensure profitBaseCosts is loaded if not already
            if (Object.keys(profitBaseCosts.mtn).length === 0) {
                ...
                if (config && config.value) {
                    profitBaseCosts = typeof config.value === 'string' ? JSON.parse(config.value) : config.value;
                }
            }
            ...
            orders.forEach(order => {
                ...
                totalProfit += (charged - baseCost);
            });
        }

        document.getElementById('metric-revenue-total').innerText = "₵" + totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        document.getElementById('metric-profit-total').innerText = "₵" + totalProfit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        ...
        if (reportLifetimeElem) {
            reportLifetimeElem.innerText = "₵" + totalProfit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        }
...
async function loadApiSettings() {
    try {
        const { data: settings } = await supabase
            .from('app_settings')
            .select('key, value')
            .in('key', ['api_auto_mtn', 'api_auto_telecel', 'api_auto_tigo', 'api_auto_bigtime']);

        if (settings) {
            settings.forEach(s => {
                if (s.key === 'api_auto_mtn') document.getElementById('apiToggleMtn').value = s.value;
                if (s.key === 'api_auto_telecel') document.getElementById('apiToggleTelecel').value = s.value;
                if (s.key === 'api_auto_tigo') document.getElementById('apiToggleTigo').value = s.value;
                if (s.key === 'api_auto_bigtime') document.getElementById('apiToggleBigtime').value = s.value;
            });
        }
    } catch (err) {
...
async function saveApiSettings() {
    const valMtn = document.getElementById('apiToggleMtn').value;
    const valTelecel = document.getElementById('apiToggleTelecel').value;
    const valTigo = document.getElementById('apiToggleTigo').value;
    const valBigtime = document.getElementById('apiToggleBigtime').value;

    const updates = [
        { key: 'api_auto_mtn', value: valMtn },
        { key: 'api_auto_telecel', value: valTelecel },
        { key: 'api_auto_tigo', value: valTigo },
        { key: 'api_auto_bigtime', value: valBigtime }
    ];
    ...
    try {
        ...
        if (error) throw error;
        alert("✅ API Auto-Fulfillment settings carefully updated!");
    } catch (err) {
        alert("API Config Update Error: " + err.message);
    }
}

async function loadApiBalance() {
    const balEl = document.getElementById('apiWalletBalance');
    if (!balEl) return;
    
    balEl.innerText = "Checking...";
    balEl.style.color = "#64748b";

    try {
        if (!window.checkApiBalance) {
            balEl.innerText = "Error: JS Missing";
            return;
        }

        const res = await window.checkApiBalance();
        if (res.success && res.data && res.data.rawBalance !== undefined) {
            const externalBal = parseFloat(res.data.rawBalance);
            balEl.innerText = "GH₵ " + externalBal.toFixed(2);
            balEl.style.color = externalBal > 50 ? "#059669" : "#dc2626"; // Red if low
        } else if (res.success && res.data && res.data.balance !== undefined) {
            // Fallback just in case rawBalance is missing but string balance exists
            balEl.innerText = res.data.balance;
            balEl.style.color = "#0f172a";
        } else {
            balEl.innerText = "API Error";
            balEl.style.color = "#dc2626";
            console.error("API Balance Error:", res.error || res);
        }
    } catch (err) {
        balEl.innerText = "Failed";
        balEl.style.color = "#dc2626";
        console.error(err);
    }
}
...
async function loadApiKeysPage() {
    try {
        ...
        if (settings) {
            settings.forEach(s => {
                const elKey = document.getElementById('datarjustApiKey');
                const elToggle = document.getElementById('masterApiIntegrationToggle');
                
                if (s.key === 'datarjust_api_key' && elKey) elKey.value = s.value;
                if (s.key === 'datarjust_api_enabled' && elToggle) elToggle.value = s.value;
            });
        }
    } catch (err) {
...
async function saveMasterApiKeys() {
    const key = document.getElementById('datarjustApiKey').value.trim();
    const enabled = document.getElementById('masterApiIntegrationToggle').value;

    const updates = [
        { key: 'datarjust_api_key', value: key },
        { key: 'datarjust_api_enabled', value: enabled }
    ];
    ...
    try {
        ...
        if (error) throw error;
        alert("✅ Master API Settings successfully updated!");
    } catch (err) {
        alert("API Settings Update Error: " + err.message);
    }
}
...
function renderPricingMatrix(dbPrices) {
    ...
    // Always ensure the base "Per GB" (sz: 0) exists for all networks if not in DB
    NETWORKS.forEach(net => {
        const key = `${net}_0`;
        groupMap[key] = { product: net, sz: 0, stock: true, roles: {} };
    });
...
async function addPricingSize() {
    ...
    // Insert placeholders for all networks and roles for this new size
    const newRows = [];
    NETWORKS.forEach(net => {
        ROLES.forEach(role => {
            newRows.push({
    ...
    try {
        ...
        if (error) throw error;
        sizeInput.value = '';
        alert(`✅ ${size}GB size added to all networks. Now set the prices in the matrix below.`);
        loadPricingConfig();
    } catch (err) {
...
function getStatusBadge(status) {
    ...
    if (s === 'true' || s === 'completed' || s === 'received') {
        bg = '#d1fae5'; text = '#059669'; dot = '#10b981'; label = s === 'true' ? 'COMPLETED' : s.toUpperCase();
...
// ==========================================
// TAB: COMMISSION ADMIN
// ==========================================
async function loadCommissionRequests() {
    const tbody = document.getElementById('commRequestsBody');
    document.getElementById('commRequestsArea').style.display = 'block';
    document.getElementById('commRatesArea').style.display = 'none';
    
    tbody.innerHTML = '<tr><td colspan="7" class="state-msg">Syncing requests...</td></tr>';
    ...
    try {
        ...
        if (!data || data.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" class="state-msg">No withdrawal requests found.</td></tr>';
            return;
        }
        ...
        data.forEach(r => {
            ...
            const user = r.users?.email || 'Unknown';
            const statusStyle = r.status === 'pending' ? 'color:#f59e0b;' : (r.status === 'approved' ? 'color:#10b981;' : 'color:#ef4444;');
            
            let actionBtn = '';
            if (r.status === 'pending') {
                actionBtn = `
                    <button onclick="approveCommRequest('${r.id}')" class="action-btn btn-approve" style="padding:4px 8px; font-size:10px;">Approve</button>
                    <button onclick="rejectCommRequest('${r.id}', '${r.user_id}', ${r.amount})" class="action-btn btn-view" style="padding:4px 8px; font-size:10px; background:#ef4444;">Reject</button>
                `;
            }
            ...
            html += `
                <tr>
                    ...
                    <td><span class="type-pill" style="background:#f1f5f9; color:#475569;">${r.method.toUpperCase()}</span></td>
                    <td>${r.momo_number || '—'}</td>
                    <td style="font-weight:700; ${statusStyle}">${r.status.toUpperCase()}</td>
                    <td>${actionBtn}</td>
                </tr>
            `;
        });
        ...
        tbody.innerHTML = '<tr><td colspan="7" class="state-msg" style="color:red;">Failed to load.</td></tr>';
    }
}

async function approveCommRequest(id) {
    if (!confirm("Mark this MOMO withdrawal as APPROVED? (Ensure you have sent the cash manually)")) return;
    
    const { error } = await supabase.from('withdrawal_requests').update({ status: 'approved' }).eq('id', id);
    if (!error) loadCommissionRequests();
    else alert("Failed: " + error.message);
}

async function rejectCommRequest(id, userId, amount) {
    const reason = prompt("Reason for rejection? (User will be refunded)");
    if (reason === null) return;

    try {
        // 1. Update Request
        const { error: reqError } = await supabase.from('withdrawal_requests').update({ status: 'rejected', admin_note: reason }).eq('id', id);
        if (reqError) throw reqError;

        // 2. Refund User
        const { data: user } = await supabase.from('users').select('commission_balance').eq('id', userId).single();
        await supabase.from('users').update({ commission_balance: (user.commission_balance || 0) + amount }).eq('id', userId);
        ...
        alert("Request rejected and user refunded.");
        loadCommissionRequests();
    } catch (err) {
...
async function loadCommissionRatesAdmin() {
    const tbody = document.getElementById('commRatesBody');
    document.getElementById('commRequestsArea').style.display = 'none';
    document.getElementById('commRatesArea').style.display = 'block';

    tbody.innerHTML = '<tr><td colspan="4" class="state-msg">Syncing rates...</td></tr>';

    try {
        const { data, error } = await supabase.from('commission_rates').select('*').order('network').order('amount');
        if (error) throw error;
        ...
        data.forEach(r => {
            html += `
                <tr>
                    ...
                    <td>${r.gb_size}</td>
                    <td><input type="number" step="0.01" value="${r.amount}" onchange="updateCommRate('${r.id}', this.value)" class="admin-input" style="width:100px; padding:4px;"></td>
                    <td><button onclick="updateCommRate('${r.id}', null)" class="action-btn btn-view" style="padding:4px 8px; font-size:10px; background:#ef4444;">Del</button></td>
                </tr>
            `;
        });
        tbody.innerHTML = html;
    } catch (err) {
        tbody.innerHTML = '<tr><td colspan="4" class="state-msg">Error: ' + err.message + '</td></tr>';
    }
}

async function updateCommRate(id, newAmount) {
    if (newAmount === null) {
        if (!confirm("Delete this commission rate?")) return;
        await supabase.from('commission_rates').delete().eq('id', id);
    } else {
        await supabase.from('commission_rates').update({ amount: parseFloat(newAmount) }).eq('id', id);
    }
    loadCommissionRatesAdmin();
}
